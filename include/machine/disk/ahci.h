...
typedef struct {
    ...
    // 0x80
    hba_prdt_entry_t prdt_entry[8]; // Physical region descriptor table entries, 0 ~ 65535
...
typedef struct {
    ...
    /* Word 3-5: Obsolete CHS parameters */
    uint16_t obsolete_chs[3];

    /* Word 6: Obsolete */
    uint16_t obsolete_2;

    /* Word 7-8: Obsolete */
    uint16_t obsolete_3[2];
    ...
    /* Word 27-46: Model Number (40 ASCII characters) */
    uint8_t model_number[40];

    /* Word 47: Read/Write Multiple Support */
    uint16_t read_write_multiple;
    ...
    /* Word 51: PIO Transfer Modes */
    uint16_t pio_modes;

    /* Word 52: DMA Transfer Modes */
    uint16_t dma_modes;

    /* Word 53-59: Various fields */
    uint16_t field_53;
    uint16_t field_54;
    uint16_t field_55;
    uint16_t field_56;
    ...
    /* Word 60-61: Total User Addressable Sectors (LBA28) */
    uint32_t lba_sectors_28;

    /* Word 62: Multiword DMA Settings */
    uint16_t dma_settings;

    /* Word 63-99: Various fields */
    uint16_t reserved_1[37];

    /* Word 100-103: Total User Addressable Sectors (LBA48) */
    uint64_t lba_sectors_48;

    /* Word 104-127: Various fields */
    uint16_t reserved_2[24];

    /* Word 128-159: Security features */
    uint16_t security[32];

    /* Word 160-255: Vendor specific and other data */
    uint16_t vendor_specific[96];
} __attribute__((packed)) ahci_identify_t;
...
#define AHCI_OPERATION_HANDLING 0x0
#define AHCI_OPERATION_END_SUCCESS 0x1
#define AHCI_OPERATION_END_FAILED 0x2
#define AHCI_OPERATION_WAITING 0x3

#define AHCI_DISK_FREE 0x0
#define AHCI_DISK_IN_PROGRESS 0x1
...
typedef struct {
    ahci_device_t dev[AHCI_MAX_DEVICES];
    uint8_t count;
} ahci_manager_t;
