...
void init_slab(void){
    ...
    for (uint32_t i = 0; i < 2; i++)
    {
        ...
        for (uint32_t j = 0; j < (uint32_t)64 << i; j++)
        {
            ...
            put_page_4k(phy_addr,SLAB_START_1024 + (1UL << (39 + i)) + (j << 12)
                    ,(uint64_t)easy_phy2linear(ptable4),0);
        }
    }
}
    ...
    for (uint32_t i = 0; i < 2; i++)
    {
        ...
        if (size <= info->size){
            ...
            if(--slab->totalfree){
                ...
                {
                    uint64_t phy_addr = alloc_page_4k();
                    put_page_4k(phy_addr,info->area_start_addr + (current->id << (18 + i)) + (j << 12)
                            ,(uint64_t)easy_phy2linear(ptable4),0);
                }
                return (void*)addr;
            }
